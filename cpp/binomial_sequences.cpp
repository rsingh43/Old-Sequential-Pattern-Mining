...
void generate_sequences(unsigned int num_sequences, unsigned int min_seq_len, unsigned int max_seq_len, unsigned int num_symbols, RNG &generator, DIST &distribution, ostream &output) {
...
template <typename RNG, typename DIST>
void generate_sequences(unsigned int num_sequences, unsigned int min_seq_len, unsigned int max_seq_len, unsigned int num_symbols, unsigned int min_set_size, unsigned int max_set_size, RNG &generator, DIST &distribution, ostream &output) {
	...
	for(unsigned int ii=0; ii < num_sequences; ii++) {
		...
		for(unsigned int jj=0; jj < seq_len; jj++) {
			...
			while(itemset.size() < set_size) {
				itemset.insert(distribution(generator));
			}
...
void generate_sequences(unsigned int num_sequences, unsigned int min_seq_len, unsigned int max_seq_len, unsigned int num_symbols, bool itemset, unsigned int min_set_size, unsigned int max_set_size, ostream &output) {
	...
	binomial_distribution<unsigned int> distribution(num_symbols-1,0.5);
	
	if(itemset) {
		generate_sequences(num_sequences, min_seq_len, max_seq_len, num_symbols, min_set_size, max_set_size, generator, distribution, output);
	}
	else {
		generate_sequences(num_sequences, min_seq_len, max_seq_len, num_symbols, generator, distribution, output);
	}
}
